...
class UNetDecoder(nn.Module):
    ...
    def forward(self, skips):
        ...
        # np.save("./f1_only_b", feature_1)
        # np.save("./f2_only_b", feature_2)
        # np.save("./f3_only_b", feature_3)
        # np.save("./f4_only_b", feature_4)
        # np.save("./f5_only_b", feature_5)

        # feature_5 = skips[-5].detach().cpu().numpy()
        # rand_id = np.random.randint(0, 1000)
        # save_dir = f"./tsne_plot/boundary/feature_5/"
        # import os 
        ...
        for s in range(len(self.stages)):
            ...
            if self.deep_supervision:
                ...
                # rand_id = np.random.randint(0, 1000)
                # save_dir = f"./tsne_plot/boundary/feature_5_decoder/"
                # import os 
                # os.makedirs(save_dir, exist_ok=True)
                # np.save(os.path.join(save_dir, f"{rand_id}"), feature_5)
                ...
                # np.save("./f1_only_b_decoder", feature_1)
                # np.save("./f2_only_b_decoder", feature_2)
                # np.save("./f3_only_b_decoder", feature_3)
                # np.save("./f4_only_b_decoder", feature_4)
                # np.save("./f5_only_b_decoder", feature_5)
                # exit(0)
